...
def removeCsvFolders():
    """
    Removes the csv files after the report's generation
    """
    # Iterate over files
    files = glob.glob('./data/*')
    for f in files:
        # Remove all trailing "\" in cases like "C:\path\to\folder\folder1\"
        # and make it as "C:\path\to\folder\folder1".
        f = f.strip("\\")

        # Split this string by "\" and get the last index element to
        # get the folder name.
        fname = f.split("\\")[-1]

        # Ignore it if it's equal to something you don't want to delete
        if fname != "reports":
